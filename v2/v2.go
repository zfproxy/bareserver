...
var defaultForwardHeaders = []string{"accept-encoding", "accept-language", "sec-websocket-extensions", "sec-websocket-key", "sec-websocket-version"}
...
func Register(server *bare.BareServer) {
	bareserver = server
	server.Handle("/v2/", v2Handler)

	server.Handle("/v2/ws-meta", v2MetaHandler)

	server.Handle("/v2/ws-new-meta", v2NewMetaHandler)

	server.HandleSocket("/v2/", v2SocketHandler)
...
func v2Handler(request *bare.BareRequest, w http.ResponseWriter, options *bare.Options) (*bare.Response, error) {
...
func v2MetaHandler(request *bare.BareRequest, w http.ResponseWriter, options *bare.Options) (*bare.Response, error) {
...
func v2NewMetaHandler(request *bare.BareRequest, w http.ResponseWriter, options *bare.Options) (*bare.Response, error) {
	headersData, err := readHeaders(request)
	if err != nil {
		return nil, err
	}
...
func v2SocketHandler(request *bare.BareRequest, socket *websocket.Conn, options *bare.Options) error {
	defer socket.Close()
	...
	if err := bareserver.DB().Set(id, string(updatedMetaJSON), metaExpiration); err != nil {
